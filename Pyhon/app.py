...
def run_simulation():
    ...
    for tractor in model.tractors:
        tractor_info.append({
            ...
            "initial_position": (int(tractor.initial_position[0]), int(tractor.initial_position[1]))
        })

    # Convert NumPy array to Python list
    field_list = field.astype(int).tolist()

    # Join the rest of the data in a JSON
    results_for_json = {
        "tractors": tractor_info,
        "field_matrix": field_list,
    }

    return jsonify(results_for_json)
